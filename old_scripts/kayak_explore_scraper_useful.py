...
import flight_scraping_single_d
...
def send_email(flights, oklahoma, ok_url):
    """
    ...
    receiver = 'simonjer805@gmail.com, manuela.a.simon@gmx.de'
    message['Subject'] = "Flights to Atlanta before Zach's wedding are $" + \
    str(oklahoma) + " right now."
    message['From'] = sender
    message['To'] = receiver
    begin = 'The non-Atlanta flights are all within the period from June 1st \
    2020 to August 30th 2020. <br><br> Here is the link to the Atlanta flights: ' + \
    '<a href=\"' + ok_url + '\">Here</a><br><br>'
    flights['Price'] = ['$'+ str(round(i)) for i in flights['Price']]
    #flights['Price'] = '$' + flights['Price'].astype(str)
    html = "<html><head></head><body><p>" + begin + msgs + \
...
def check_df(results, oklahoma):
    
    filename = 'data/' + str(timeframe_begin) + '_to_' + str(timeframe_end) + '_kayak_scrape.csv'
    ...
    current_scrape.set_value('Oklahoma',oklahoma)
...
filename = 'data/' + str(timeframe_begin) + '_to_' + str(timeframe_end) + '_all_flights.csv'
now = datetime.datetime.now()
...
oklahoma, ok_url = flight_scraping_single_d.run_scraper()
msgs, email = check_df(results, oklahoma)

if email:
    send_email(results, oklahoma, ok_url)
#%%
